...
### 3.3 主机 -> 从机 (编码器诊断同步)
**Type**: `admin_encoder` (用于维护 -> 编码器诊断页面)

| 字段 (JSON Key) | 类型 | 物理含义 | 备注 |
| :--- | :--- | :--- | :--- |
| `type` | string | 数据包类型 | 固定为 "admin_encoder" |
...
**示例**: `{"type":"admin_encoder","data":{"pulse_count":12345,"velocity":20.5,"status":1,"raw_val":12345,"corrected_val":12340,"logic_val":50,"zero_count":10,"zero_correct":9,"zero_total":10}}`

### 3.4 主机 -> 从机 (激光扫描仪诊断同步)
**Type**: `admin_laser` (用于激光扫描仪诊断页面)

| 字段 (JSON Key) | 类型 | 物理含义 | 备注 |
| :--- | :--- | :--- | :--- |
| `type` | string | 数据包类型 | 固定为 "admin_laser" |
...
**示例**: `{"type":"admin_laser","data":{"states":1,"history_p1":"ABC...","history_p2":"...","history_p3":"...","history_p4":"...","history_p5":"..."}}`

---

### 3.5 主机 -> 从机 (下料口配置同步)
**Type**: `admin_outlets` (用于下料口配置页面)

| 字段 (JSON Key) | 类型 | 物理含义 | 备注 |
| :--- | :--- | :--- | :--- |
| `type` | string | 数据包类型 | 固定为 "admin_outlets" |
| `data` | array | 8 路出口配置数组 | 长度固定为 8 |
| `data[].min` | float | 最小匹配直径 | 单位：mm |
| `data[].max` | float | 最大匹配直径 | 单位：mm |
| `data[].mask` | int | 长度等级位掩码 | Bit 0:S, 1:M, 2:L (1=选中, 0=未选) |

**示例**: `{"type":"admin_outlets","data":[{"min":18.0,"max":20.0,"mask":7}, ...]}`
